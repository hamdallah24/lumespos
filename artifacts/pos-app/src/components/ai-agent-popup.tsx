            ...
            <div className="flex-1 overflow-y-auto px-5 py-3 space-y-3 min-h-0">
              ...
              {messages.map((m, i) => (
                <div key={i} className={`flex gap-2 ${m.role === "user" ? "justify-end" : ""}`}>
                  {m.role === "assistant" && (
                    <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#1565FF] to-[#0A4CD0] flex items-center justify-center shrink-0">
                  ...
                  <div className={`max-w-[80%] rounded-2xl px-3.5 py-2.5 text-sm ${
                    ...
                    <p className="whitespace-pre-wrap leading-relaxed">{m.text || (loading && i === messages.length - 1 ? "..." : "")}</p>
                  </div>
                  {m.role === "user" && (
                    <div className="w-7 h-7 rounded-lg bg-[#1565FF]/10 flex items-center justify-center shrink-0">
