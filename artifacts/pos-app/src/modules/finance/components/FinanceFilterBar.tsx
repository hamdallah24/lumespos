...
import { useFinanceFilter, type DatePreset, type ComparisonMode } from "../context/FinanceFilterContext";
...
export default function FinanceFilterBar() {
  ...
  return (
    <div className="sticky top-0 z-30 bg-background/95 backdrop-blur border-b border-border -mx-4 px-4 py-2 flex items-center gap-2 overflow-x-auto">
      {/* Branch Filter */}
      <div className="relative group shrink-0">
        <button className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg border border-border hover:bg-muted/50 transition-colors whitespace-nowrap">
        ...
        <div className="absolute top-full left-0 mt-1 bg-card border border-border rounded-xl shadow-lg p-1.5 min-w-[180px] hidden group-hover:block z-50">
          <button
            onClick={() => setBranches([])}
      ...
      <div className="relative group shrink-0">
        ...
        <div className="absolute top-full left-0 mt-1 bg-card border border-border rounded-xl shadow-lg p-1.5 hidden group-hover:block z-50">
          {DATE_PRESETS.map(p => (
            <button
              key={p.value}
          ...
          {state.datePreset === "custom" && (
            <div className="px-2 pt-2 pb-1 space-y-1 border-t border-border mt-1">
              <input
                ...
                onChange={e => setCustomDates(e.target.value, state.endDate || "")}
                className="w-full text-xs border border-border rounded-md px-2 py-1 bg-background"
              />
              <input
                type="date"
                value={state.endDate || ""}
                onChange={e => setCustomDates(state.startDate || "", e.target.value)}
                className="w-full text-xs border border-border rounded-md px-2 py-1 bg-background"
              />
            </div>
          )}
        ...
        <div className="relative group shrink-0">
          ...
          <div className="absolute top-full left-0 mt-1 bg-card border border-border rounded-xl shadow-lg p-1.5 hidden group-hover:block z-50">
            ...
            {periods.map((p: any) => (
              <button
                key={p.id}
                onClick={() => setAccountingPeriod(p.id)}
      ...
      {state.branchIds.length > 0 || state.datePreset !== "thisMonth" ? (
        <span className="text-[10px] text-muted-foreground whitespace-nowrap">
          {state.branchIds.length > 0 ? `${state.branchIds.length} cabang` : "Semua"}
          {" · "}
