...
import type { DatePreset } from "../PlatformFilterTypes";
import { useQueryClient } from "@tanstack/react-query";
import { ChevronDown, RefreshCw } from "lucide-react";

export type FilterBarMode = "full" | "compact" | "minimal";
...
export default function PlatformFilterBar({
  ...
  const { state, setBranches, setDatePreset, setCustomDates, setAccountingPeriod, refresh } = usePlatformFilter();
      ...
      {showBranch && (
        <div className="relative shrink-0">
          <button
            ...
            className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg border border-border hover:bg-muted/50 transition-colors whitespace-nowrap"
          >
            ...
            <>
              ...
              <div className="absolute top-full left-0 mt-1 bg-card border border-border rounded-xl shadow-lg p-1.5 z-50">
                {DATE_PRESETS.map((p) => (
                  <button
                    key={p.value}
                    onClick={() => { setDatePreset(p.value as DatePreset); close(); }}
        ...
        <div className="relative shrink-0">
          ...
          {openMenu === "period" && (
            <>
              ...
              <div className="absolute top-full left-0 mt-1 bg-card border border-border rounded-xl shadow-lg p-1.5 z-50">
                <button
                  onClick={() => { setAccountingPeriod(null); close(); }}
                ...
                {periods.map((p) => (
                  <button
                    key={p.id}
                    onClick={() => { setAccountingPeriod(p.id); close(); }}
