          ...
          <TabsContent value="products" className="flex-1 overflow-hidden flex flex-col mt-3 md:mt-4 gap-3">
            ...
            <ScrollArea className="flex-1">
              ...
              {filtered.map((p, idx) => (<div key={p.id} className={`flex items-center gap-2 px-3 py-2.5 hover:bg-muted/30 transition-colors cursor-pointer ${idx < filtered.length - 1 ? "border-b" : ""}`} onClick={() => showVariants(p)}>
                ...
                <div className="shrink-0 text-right"><p className="text-[11px] text-muted-foreground">{formatRp((p as any).costPrice ?? 0)}</p><p className="font-bold text-sm text-primary">{(p as any).hasVariants ? `Rp${Number((p as any).minPrice).toLocaleString("id-ID")} - Rp${Number((p as any).maxPrice).toLocaleString("id-ID")}` : formatRp(p.price)}</p></div>
