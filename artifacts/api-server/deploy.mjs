    ...
    conn.sftp((err, sftp) => {
      ...
      function uploadNext() {
        if (idx >= files.length) {
          ...
          sftp.end();
          conn.exec("cd /home/ubuntu/lumespos && pnpm remove express-async-errors --filter @workspace/api-server 2>&1; pm2 restart pos-api --update-env 2>&1", (e2, s2) => {
            if (e2) { console.error("Restart error:", e2); conn.end(); process.exit(1); return; }
            let o2 = "";
            s2.on("data", d => o2 += d);
            s2.on("close", () => {
              console.log("PM2 restart:", o2);
              conn.end();
        ...
        const remotePath = `${REMOTE}/dist/${f}`;
