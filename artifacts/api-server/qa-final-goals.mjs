...
async function run() {
  ...
  c.on("ready", async () => {
    ...
    await exec(c, `curl -s -m 10 -c /tmp/qa_gl.txt -X POST 'http://localhost:3000/api/auth/login' -H 'Content-Type: application/json' -d '{"email":"admin@lumespos.com","password":"admin123"}' > /dev/null`);
    ...
    const resp = await exec(c, `curl -s -m 300 -b /tmp/qa_gl.txt -X POST 'http://localhost:3000/api/ai/chat' -H 'Content-Type: application/json' -d '{"message":"buat misi cari dan perbaiki bug di file execution-pipeline.ts","mode":"ceo"}' 2>&1 | grep -E "type.*meta|type.*done|editFile|writeFile" | head -6`);
    ...
    if (before !== after) {
      ...
      const logs = await exec(c, `pm2 logs pos-api --nostream --lines 500 2>&1 | grep -E "GOV:DEBUG|PIPELINE:.*cycle|APPROVED|PERSETUJUAN|editFile|writeFile" | tail -15`);
