...
export async function getHealthData(branchId: number): Promise<HealthData> {
  ...
  const currentMonth = await getMonthlyTotals(branchId, 0);
  const lastMonth = await getMonthlyTotals(branchId, 1);
