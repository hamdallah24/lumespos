...
import { EXECUTION_COMPLETION_POLICY } from "./execution-policy";
...
class ExecutionGovernor {
  ...
  afterCycle(
    ...
    if (hasToolCalls) {
      ...
      if (pendingGoals.length > 0) {
        ...
        if (cycles >= this._evidenceThreshold
            && this.metrics.evidenceQuality >= EXECUTION_COMPLETION_POLICY.minimumEvidence) {
    ...
    this._onEvent?.({
      ...
      timestamp: Date.now(),
      progress: {
        // Goal tree: completed/total goals × 100
        assignment: 100,
        execution: this.goalTree.progress(),
        overall: this.goalTree.progress(),
      },
