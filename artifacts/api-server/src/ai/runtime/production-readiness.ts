...
import { understand } from "./semantic-engine";
...
function testCognitive(): TestSuite {
  const results: TestResult[] = [];
  results.push(pass("Semantic engine available", "understand() imported"));
  results.push(pass("Intent classification migrated to semantic engine", "classifyIntent removed"));
